...
describe('Frequency', function () {
  describe('#getStandardConversion(quantity)', function () {
    ...
    it('should return 1000 kilohertz when quantity is 1 megahertz', function () {
      ...
      assert.strictEqual(
        (megahertz_in_hz * unit_object_khz.ratio).toFixed(0),
        '1000'
      );
    });
    ...
    // Non-nominal tests
    it('should handle a very large frequency value without precision errors', function () {
      assert.strictEqual(getStandardConversion(unit_object_hz, 1e12), 1e12);
    });

    it('should handle floating-point values accurately for kilohertz', function () {
      assert.strictEqual(
        getStandardConversion(unit_object_khz, 0.123).toFixed(3),
        '123.000'
      );
    });

    it('should handle an extremely large frequency value in megahertz', function () {
      assert.strictEqual(getStandardConversion(unit_object_mhz, 1e6), 1e12);
    });
  });
});

...
function convertTime(inputTimeString, offset, toTimeZone) {
  ...
  const convertedTime = inputTime.toLocaleString('en-US', {
    timeZone: toTimeZone,
  });
...
function getTzString(tz) {
  const timeZoneInfo = this.timezones.find(
    (zone) => zone.aliases.includes(tz) || zone.zone === tz
  );
...
function getAllTimeConversions(inputString) {
  ...
  const cleanedStrings = inputString.map((input) => {
    ...
    timezones.forEach((timezone) => {
      const aliasFound = timezone.aliases.find((alias) =>
    ...
    if (fromTzString) {
      timezones.forEach((zone) => {
        if (zone.tz_string !== fromTzString) {
          ...
          res +=
            `${zone.zone}: ` + String(convertedTime).replace(',', '') + ',';
        }
      });
    } else {
